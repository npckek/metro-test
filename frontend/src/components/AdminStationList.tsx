          ...
          <TableBody>
            {stations.map((station) => (
              <TableRow key={station.id}>
                ...
                <TableCell>
                  X: {station.coordinates[0] || "N/A"}
                  <br /> Y: {station.coordinates[1] || "N/A"}
                </TableCell>
                <TableCell className="text-center">
                  {station.entrance_cnt_7}, {station.entrance_cnt_8},{" "}
                  {station.entrance_cnt_9}
                </TableCell>
                <TableCell className="text-center">
                  {station.entrance_cnt_17}, {station.entrance_cnt_18},{" "}
                  {station.entrance_cnt_19}
                </TableCell>
                <TableCell className="text-center">
                  {station.exit_cnt_7}, {station.exit_cnt_8},{" "}
                  {station.exit_cnt_9}
                </TableCell>
                <TableCell className="text-center">
                  {station.exit_cnt_17}, {station.exit_cnt_18},{" "}
                  {station.exit_cnt_19}
                </TableCell>
                <TableCell className="text-right space-x-2">
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      let ok = confirm(
                        `Вы уверены, что хотите удалить станцию №${station.station_id}?`,
                      );
                      if (!ok) return;
