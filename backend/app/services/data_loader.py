...
def load_data_from_geojson():
    ...
    geojson_path = os.path.join(os.path.dirname(__file__), '..', '..', 'data', 'stations.geojson')
    ...
    for feature_dict in data.get("features", []):
        ...
        if db.query(MetroStation).filter(MetroStation.station_id == props["station_id"]).first():
            continue 
        ...
        try:
            ...
            db.rollback()
            print(f"WARNING: Integrity error for station_id {props.get('station_id')}. Skipped.")
        except Exception as e:
            db.rollback()
            print(f"FATAL ERROR: Failed to process station {props.get('station_name')}. Details: {type(e).__name__}: {e}")
